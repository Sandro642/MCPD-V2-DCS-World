...
-- Filtre de couleur (couleur bleue sur fond de carte)
local function applyColorFilterToMap()
    local color_filter = CreateElement "ceMeshPoly"
    color_filter.name = "color_filter"
    color_filter.isdraw = true
    color_filter.material = "color_filter_material"
    color_filter.vertices = verts
    color_filter.indices = default_box_indices
    color_filter.primitivetype = "triangles"
    color_filter.screenspace = 2
    color_filter.additive_alpha = true
    color_filter.collimated = false
    Add(color_filter)
end

-- Application du filtre de couleur lors du rendu de la carte
applyColorFilterToMap()
...
function SA_PPLI_Symbol(parent, course_controller, isOutlined, sa_format)
	...
	addStrokeSymbol(parent.."_PPLI_DT2", {"stroke_symbols_MDI_AMPCD", "SA-DT2"}, "FromSet", nil, parent, {{"MPD_SA_PPLI_DT2"}, {"MPD_SA_PPLI_Color", isOutlined}})
